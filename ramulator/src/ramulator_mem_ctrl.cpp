...
void RamulatorMemory::readComplete(ramulator::Request& req) {
    std::multimap<uint64_t, RamulatorAccEvent*>::iterator it = inflightRequests.find(req.addr);
...
void RamulatorMemory::enqueue(RamulatorAccEvent* ev, uint64_t cycle) {
    ...
    Request req(ev->getAddr(), req_type, cb_func, 0);
    assert(ev->getAddr()==req.addr);

    if(wrapper->send(req) == false)  
        waiting_req.push_back(req);

    inflightRequests.insert(std::pair<Address, RamulatorAccEvent*>(ev->getAddr(), ev));
    ev->hold();
}
