...
MemObject* BuildMemoryController(Config& config, uint32_t lineSize, uint32_t frequency, uint32_t domain, g_string& name) {
    ...
    //HJHJ
        ...
        uint64_t cpuFreqHz = 1000000 * frequency;
        string ramulator_config = config.get<const char*>("sys.mem.ramulator_config");
        uint32_t ramulator_stat_interval = config.get<uint32_t>("sys.mem.ramulator_stat_interval", 1000000);
        string statsPrefix = config.get<const char*>("sim.statsPrefix");
        statsPrefix += ".";

        mem = new RamulatorMemory(ramulator_config, ramulator_stat_interval, statsPrefix, cpuFreqHz, latency, domain, name);
    } else {
